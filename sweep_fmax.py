...
def build(board, logs_dir, dir, curr_seed, bar):
    ...
    try:
        ...
        pattern = r"'(?P<clock>.*)': (?P<freq>[0-9.]+ \w?Hz)"
        max_freq = None
        with open(log_file, "w") as out_f:
            out_f.write(result.stdout.decode())
            for line in result.stdout.decode().splitlines():
                if not line.startswith("Info: Max frequency"):
                    continue
                for match in re.finditer(pattern, line):
                    groupdict = match.groupdict()
                    freq_float, freq_unit = groupdict["freq"].split()
                    assert freq_unit == "MHz"
                    freq_float = float(freq_float)
                    if not max_freq or freq_float < max_freq:
                        max_freq = freq_float
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=os.process_cpu_count()) as executor:
                    ...
                    for future in concurrent.futures.as_completed(tasks):
                        ...
                        if future.result() is not None:
                            count += 1
                            ok_seeds.append(future.result())
                        else:
                            failed_seeds.append(tasks[future])

            if len(ok_seeds) == len(items):
